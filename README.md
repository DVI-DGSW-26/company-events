...
| `EVENTS_API` | `https://api.dvi-ind.com/events` | 등록·수정 기능에 필요 |
...
행사 정보와 사진은 **사내 행사 서버**(`api.dvi-ind.com/events`)에 보관합니다.
저장하면 바로 반영되고, 별도 빌드나 배포를 기다리지 않습니다.

```
화면에서 저장
 └─ /api/admin/*  (Vercel)   로그인·권한 확인 후 그대로 전달
      └─ 사내 행사 서버에 저장
           └─ 화면이 /api/archive 로 읽어 바로 표시
```

사진 주소는 서명이 붙은 임시 주소라 시간이 지나면 만료됩니다. 사진이 안 뜨면
화면이 목록을 한 번 다시 받아 새 주소로 바꿉니다. 따로 하실 일은 없습니다.
...
Vercel 환경변수에 아래 하나만 넣고 **Redeploy** 하면 됩니다.
...
> 환경변수는 넣는 것만으로 반영되지 않습니다. 이미 돌고 있는 배포는 만들어질 때의
> 값을 쓰기 때문에, 값을 저장한 **뒤에** Redeploy 해야 합니다.

### 잘 들어갔는지 확인

로그인한 상태에서 `/api/health` 를 열면 지금 상태가 그대로 보입니다.

| `행사자료를_읽는_곳` | 뜻 |
|---|---|
| `사내 행사 서버 (전환 완료)` | 정상 |
| `배포에 포함된 기준 자료…` | `EVENTS_API` 미설정 또는 Redeploy 안 함 |

`사내_행사_서버` 가 `거절 (401)` 이면 로그아웃 후 다시 로그인하세요.

### 서버를 읽지 못할 때

목록은 배포에 포함된 기준 자료(행사 12건)로 떨어져 **보기와 내려받기는 계속됩니다.**
등록·수정만 막힙니다. 화면 아래에 이유가 표시됩니다.
...
```jsonc
{
  ...
  "date": "2026-09-01",
  "title": "행사 이름",
  "subtitle": "수상·성과 (받은 상이나 이 행사에서의 성과)",
  "place": "장소",
  "host": "주관 기관",
  "attendees": "참석자",
  "summary": "행사 개요 한두 문장",
