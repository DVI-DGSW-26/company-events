 ...
 *
 * 서버가 내려주는 서명 주소의 호스트가 이 사이트 주소로 찍혀 오는 일이 있다.
 * 그대로 두면 브라우저가 Vercel 로 요청해 사진이 깨지고 내려받기가 실패한다.
 * 서명은 경로·만료를 묶은 것이라 호스트만 바꿔도 그대로 유효하다.
 *
 * 응답 모양이 바뀌어도 놓치지 않도록 값 전체를 훑는다.
 */
